...
export function RoomSearchBar(props: RoomSearchProps) {
    ...
    return (
        <Box
            ...
            border={1} px={8} py={4} mb={4} style={{ background: "rgba(0, 0, 0, 0.5)" }}
        >
            ...
            <Button style={{textTransform: 'none'}} variant="contained" onClick={handleRoomSearch}>
                {loading ? <Typography>Searching...</Typography> : <Typography>Search</Typography>}
            </Button>
            {error && <Typography color='red'>{error.message}</Typography>}
            {validationError && <Typography color='red'>{validationError}</Typography>}
        </Box>
    );
}
